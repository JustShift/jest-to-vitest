...
// Release-gate "lint rule": the converter must never reach for code execution.
// This guards the no-execution promise at the source level — the ShiftKit
// production CSP omits 'unsafe-eval', so the browser worker cannot use these
// either (mirrors the same gate in @shiftkit/webpack-to-vite).
const srcDir = join(dirname(fileURLToPath(import.meta.url)), '..', 'src');

const tsFiles = readdirSync(srcDir).filter((f) => f.endsWith('.ts'));

describe('no code execution in the converter source', () => {
