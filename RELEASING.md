...
Watch a run: <https://github.com/JustShift/jest-to-vitest/actions>.
...
- Actions run is green: <https://github.com/JustShift/jest-to-vitest/actions>
- npm shows the new version: `npm view @shiftkit/jest-to-vitest version`
- GitHub Release exists: <https://github.com/JustShift/jest-to-vitest/releases>
...
1. **First 72 hours, no dependents:** `npm unpublish @shiftkit/jest-to-vitest@<version>` — last resort; it fragments the version timeline.
2. **Otherwise — deprecate, don't unpublish:**
   ```bash
   npm deprecate @shiftkit/jest-to-vitest@0.2.1 "Critical bug; use 0.2.2 instead"
   ```
...
- [ ] Confirm npm Trusted Publishing is configured once on npmjs.com → `@shiftkit/jest-to-vitest` → Settings → Trusted Publishing → GitHub Actions → repo `JustShift/jest-to-vitest`, workflow `release.yml`.
